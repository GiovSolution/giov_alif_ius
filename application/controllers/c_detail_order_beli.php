...
class C_detail_order_beli extends CI_Controller {
	
	function __construct(){
		parent::__construct();		
		$this->load->model('m_detail_order_beli', '', TRUE);
	}
	...
	function save(){
		/*
		 * Collect Data ==> diambil dari [model.detail_order_beli]
		 */
		 ...
		 * Processing Data
		 */
		$result = $this->m_detail_order_beli->save($data);
		echo json_encode($result);
	}
	...
	function delete(){
		 ...
		 * Processing Data
		 */
		$result = $this->m_detail_order_beli->delete($data);
		echo json_encode($result);
	}
	...
	function export2Excel(){
		...
		foreach($data as $record)
		{
			...
			foreach ($data[0] as $key => $value)
			{
				...
				if($key == strtoupper('detail_order_beli')){
		...
		$filename='detail_order_beli.xlsx'; //save our workbook as this file name
	...
	function export2PDF(){
		...
		$this->html2pdf->filename('detail_order_beli.pdf');
		
		//Set the paper defaults
		$this->html2pdf->paper('a4', 'portrait');
		
		//Load html view
		$this->html2pdf->html($this->load->view('pdf_detail_order_beli', $data, true));
	...
	function printRecords(){
		...
		$data["records"] = $getdata;
		$data["table"] = "detail_order_beli";
		$print_view=$this->load->view("p_detail_order_beli.php",$data,TRUE);
		if(!file_exists("temp")){
			mkdir("temp");
		}
		$print_file=fopen("temp/detail_order_beli.html","w+");
