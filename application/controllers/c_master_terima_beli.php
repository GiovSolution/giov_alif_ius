...
class C_master_terima_beli extends CI_Controller {
	...
	function getAll(){
		 ...
		 * Processing Data
		 */
		$result = $this->m_master_terima_beli->getAll($start, $page, $limit, $query, $filter);
		echo json_encode($result);
	}
	
	function save(){
		/*
		 * Collect Data ==> diambil dari [model.master_terima_beli]
		 */
		 ...
		 * Processing Data
		 */
		$result = $this->m_master_terima_beli->save($data);
		echo json_encode($result);
	}
	...
	function delete(){
		 ...
		 * Processing Data
		 */
		$result = $this->m_master_terima_beli->delete($data);
		echo json_encode($result);
	}
	...
	function export2Excel(){
		...
		foreach($data as $record)
		{
			...
			foreach ($data[0] as $key => $value)
			{
				...
				if($key == strtoupper('master_terima_beli')){
		...
		$filename='master_terima_beli.xlsx'; //save our workbook as this file name
	...
	function export2PDF(){
		...
		$this->html2pdf->filename('master_terima_beli.pdf');
		
		//Set the paper defaults
		$this->html2pdf->paper('a4', 'portrait');
		
		//Load html view
		$this->html2pdf->html($this->load->view('pdf_master_terima_beli', $data, true));
	...
	function printRecords(){
		...
		$data["records"] = $getdata;
		$data["table"] = "master_terima_beli";
		$print_view=$this->load->view("p_master_terima_beli.php",$data,TRUE);
		if(!file_exists("temp")){
			mkdir("temp");
		}
		$print_file=fopen("temp/master_terima_beli.html","w+");
		fwrite($print_file, $print_view);
		echo '1';
	}
	
	function printForm(){
		$terima_id = ($this->input->post('terimaid', TRUE) ? $this->input->post('terimaid', TRUE) : 0);
		
		//$info = $this->m_public_function->get_info();
		$result = $this->m_master_terima_beli->printForm($terima_id);
		$data["records"] = $result;
		//$data["info"] = $info;
		$data["table"] = "t_jual";
		$print_view=$this->load->view("p_master_terima_beli_form.php",$data,TRUE);
		if(!file_exists("temp")){
			mkdir("temp");
		}
		$print_file=fopen("temp/master_terima_beli_form_".$this->session->userdata('user_id').".html","w+");
		fwrite($print_file, $print_view);
