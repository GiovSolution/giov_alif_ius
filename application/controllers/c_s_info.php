...
class C_s_info extends CI_Controller {
	
	function __construct(){
		parent::__construct();		
		$this->load->model('m_s_info', '', TRUE);
	}
	...
	function getAll(){
		 ...
		 * Processing Data
		 */
		$result = $this->m_s_info->getAll($start, $page, $limit);
		echo json_encode($result);
	}
	
	function save(){
		/*
		 * Collect Data ==> diambil dari [model.s_info]
		 */
		 ...
		 * Processing Data
		 */
		$result = $this->m_s_info->save($data);
		echo json_encode($result);
	}
	...
	function delete(){
		 ...
		 * Processing Data
		 */
		$result = $this->m_s_info->delete($data);
		echo json_encode($result);
	}
	...
	function export2Excel(){
		...
		$filename='s_info.xlsx'; //save our workbook as this file name
	...
	function export2PDF(){
		...
		$this->html2pdf->filename('s_info.pdf');
		
		//Set the paper defaults
		$this->html2pdf->paper('a4', 'portrait');
		
		//Load html view
		$this->html2pdf->html($this->load->view('pdf_s_info', $data, true));
	...
	function printRecords(){
		...
		$data["records"] = $getdata;
		$data["table"] = "s_info";
		$print_view=$this->load->view("p_s_info.php",$data,TRUE);
		if(!file_exists("temp")){
			mkdir("temp");
		}
		$print_file=fopen("temp/s_info.html","w+");
